...
mean_scores = np.mean(test_scores_all, axis=0)  # mean (N, 10)
mean_scores = torch.from_numpy(mean_scores).float().to(device)  # mean (N, 10)
...
for batch_prev_items, batch_locale, batch_candidate_set, batch_len, batch_mask, \
    ...
    end_ind = start_ind + batch_size if start_ind + batch_size < len(mean_scores) else len(mean_scores)
    ind += 1
    scores = mean_scores[start_ind: end_ind, :]
...
# logger.info('Save score to {}, shape = {}'.format('output/{}_{}_{}.npy'.format(seed, exp_id, model_name), test_scores_all.shape))
# np.save('output/scores_{}_{}_{}_expids{}.npy'.format(seed, exp_id, model_name, str(load_exp_id)), test_scores_all)
# np.save('output/preds_all_{}_{}_{}_expids{}.npy'.format(seed, exp_id, model_name, str(load_exp_id)), test_res_all)
# np.save('output/preds_{}_{}_{}_expids{}.npy'.format(seed, exp_id, model_name, str(load_exp_id)), test_res)
...
#     train_res.append(sorted_candidate_set.detach().cpu().numpy())
# train_scores = np.concatenate(train_scores, axis=0)

# logger.info('Calculating MRR...')
# train_res = np.concatenate(train_res, axis=0)
# train_mrr_list = []
# for ind, x in tqdm(enumerate(train_res), total=len(train_res)):
#     x = list(x)
#     if len(x) < 100:
#         for i in top200:
#             if i not in x:  # and i not in prev_items(需要单独load)
#                 x.append(i)
#             if len(x) >= 100:
#                     break
#     # x: 100个候选结构
#     assert len(x) == 100
#     label = df_train_encoded.iloc[ind]['next_item']
#     try:
#         pred_result = x.index(label)
#         train_mrr_list.append(1 / (pred_result + 1))
#     except:
#         train_mrr_list.append(0)
# logger.info('Full Train Set MRR is {}'.format(np.mean(train_mrr_list)))
